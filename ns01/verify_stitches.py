...
# The oval belly patch Rnd 1 is worked around a foundation chain, not around a
...
if __name__ == "__main__":
    ...
    for name, (rows, copies) in ALL.items():
        ...
        for idx, (label, instr, stated, note) in enumerate(rows):
            if idx == 0:
                prev = 0
            if name == "belly" and idx == 0:
                o = oval_foundation(9)
                prod = o["total"]
                ok = prod == stated
                print(f"  {label:<8} ch 9 oval -> 1+{o['front_mid']}+3+{o['back_mid']}+2 "
                      f"= {prod}  stated ({stated})  {'OK' if ok else 'MISMATCH'}")
                if not ok: bad += 1
            ...
            print(f"  {label:<8} {instr:<38} stated ({stated:>3})  produced {p:>3}  "
                  f"{'OK' if ok and cons_ok else 'MISMATCH'}{cons}")
            prev = stated
            total += 1
        print()
    print("=" * 70)
    print(f"NS01 Hamish: {total} rounds checked, {bad} problems")
