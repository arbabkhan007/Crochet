...
if __name__ == "__main__":
    from pieces import ALL, SPUD_FOUNDATION
    total = bad = 0
    for name, (rows, copies) in ALL.items():
        ...
        for idx, (label, instr, stated, note) in enumerate(rows):
            if idx == 0:
                prev = 0
            if name == "spud" and idx == 0:
                o = oval_foundation(SPUD_FOUNDATION)
                prod = o["total"]
                ok = prod == stated
                print(f"  {label:<8} ch {SPUD_FOUNDATION} oval -> "
            ...
            shown = instr if len(instr) < 52 else instr[:49] + "..."
            print(f"  {label:<8} {shown:<52} stated ({stated:>3}) produced {p:>3} "
                  f"consumed {c:>3}  {'OK' if ok and cons_ok else 'MISMATCH'}{cons}")
            prev = stated
            total += 1
        print()
    print("=" * 70)
    print(f"NS07 Trio: {total} rounds/rows checked, {bad} problems")
